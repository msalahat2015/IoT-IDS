...
class BilstmClassifier(BaseAlgorithm):
    """
    A Stacking Classifier that can handle multiple layers of stacking, including a BiLSTM model.
    """
    ...
    def _build_tf_model(self, est_param):
        ...
        for layer_config in layers_config:
            ...
            if class_name == 'tensorflow.keras.layers.Bidirectional':
                # Handle BiLSTM
                lstm_config = config.get('layer', {}).get('config', {})
                lstm_units = lstm_config.get('units', 64)
                lstm_activation = lstm_config.get('activation', 'relu')
                lstm_return_sequences = lstm_config.get('return_sequences', False)
                lstm_input_shape = lstm_config.get('input_shape')
